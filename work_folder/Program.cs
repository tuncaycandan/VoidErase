...
internal static class Program
{
    ...
    internal static void DestroyFileSilent(string path)
    {
        ...
        byte[] headerNonce = RandomNumberGenerator.GetBytes(12);
        ...
        string source, string destination, byte[] key, byte[] headerNonce)
    {
        ...
        output.Write(new byte[] { (byte)'P', (byte)'D', (byte)'S', (byte)'1' });
        output.WriteByte(1);
        output.Write(BitConverter.GetBytes(ChunkSize));
        output.Write(BitConverter.GetBytes(total));
        ...
        while (true)
        {
            ...
            aes.Encrypt(nonce, plain.AsSpan(0, read),
                cipher.AsSpan(0, read), tag);
        ...
        CryptographicOperations.ZeroMemory(sourceDigest);
        output.Flush(true);
        ...
        string path, byte[] key, byte[] expectedHeaderNonce)
    {
        ...
        ReadExactly(fs, b8);
        long total = BitConverter.ToInt64(b8);
        ReadExactly(fs, b8);
        long chunks = BitConverter.ToInt64(b8);

        byte[] headerNonce = new byte[12];
        ReadExactly(fs, headerNonce);

        if (!CryptographicOperations.FixedTimeEquals(headerNonce, expectedHeaderNonce))
        ...
        {
            ReadExactly(fs, b4);
            int length = BitConverter.ToInt32(b4);
            ...
            ReadExactly(fs, tag);
            ReadExactly(fs, cipher.AsSpan(0, length));

            aes.Decrypt(nonce, cipher.AsSpan(0, length),
                tag, plain.AsSpan(0, length));
...
internal sealed class ShellDestroyForm : Form, IProgressReporter
{
    ...
    public void ReportProgress(long processed, long total, TimeSpan elapsed)
    {
        ...
        int percent = total == 0 ? 100 :
            (int)Math.Clamp(processed * 100L / total, 0, 100);
    ...
    public void ReportValidation(long current, long total, TimeSpan elapsed)
    {
        ...
        int percent = total == 0 ? 100 :
            (int)Math.Clamp(current * 100L / total, 0, 100);
...
internal sealed class MainForm : Form, IProgressReporter
{
    ...
    private static bool IsContextMenuKeyInstalled(string menuKey)
    {
        ...
        return !string.IsNullOrWhiteSpace(command) && command.Contains("--destroy", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private async Task CheckForUpdatesAsync(bool interactive)
    {
        ...
        try
        {
            ...
            {
                if (string.Equals(asset.GetProperty("name").GetString(), "VoidErase.exe", StringComparison.OrdinalIgnoreCase))
                {
                    url = asset.GetProperty("browser_download_url").GetString();
                    digest = asset.TryGetProperty("digest", out JsonElement d) ? d.GetString() : null;
                    break;
                }
            }
            ...
            await using FileStream dest = new(temp, FileMode.CreateNew, FileAccess.Write, FileShare.None);
            ...
            if (!string.IsNullOrWhiteSpace(digest) && digest.StartsWith("sha256:", StringComparison.OrdinalIgnoreCase))
            {
                ...
                string actual = Convert.ToHexString(await SHA256.HashDataAsync(verify));
                if (!string.Equals(actual, digest[7..].Trim(), StringComparison.OrdinalIgnoreCase)) throw new CryptographicException(L.T("İndirilen dosyanın SHA-256 doğrulaması başarısız oldu.", "Downloaded file SHA-256 verification failed."));
            }
    ...
    public void ReportProgress(long processed, long total, TimeSpan elapsed)
    {
        ...
        int percent = total == 0 ? 100 : (int)Math.Clamp(processed * 100L / total, 0, 100);
        SetProgress(percent);
        double seconds = Math.Max(elapsed.TotalSeconds, 0.001);
        double mbps = processed / 1024d / 1024d / seconds;
        long remaining = Math.Max(0, total - processed);
    ...
    public void ReportValidation(long current, long total, TimeSpan elapsed)
    {
        ...
        int percent = total == 0 ? 100 : (int)Math.Clamp(current * 100L / total, 0, 100);
        SetProgress(percent);
